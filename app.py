...
# Import AI prediction function mula sa predictor.py
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # Save to history buffer (Temp, pH, DO)
        current_features = [
...
def predict_ai():
    ...
    try:
        # Siguraduhing may 10 readings sa buffer
        current_features = [
            float(latest_sensor_data["temp"]),
            float(latest_sensor_data["ph"]),
            float(latest_sensor_data["do"])
        ...
        # Kunin ang huling 10 readings ([Temp, pH, DO])
        last_10 = list(sensor_history)

        # Patakbuhin ang prediction mula sa predictor.py
        result = predict_water_quality(last_10)

        # Tignan kung Safe ang lahat ng parameters
        all_safe = all('Safe' in result[key]['svm_status'] for key in ['temp', 'ph', 'do'])
